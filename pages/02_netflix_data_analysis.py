...
min_year = movies_df["release_year"].min() if movies_df is not None else None
max_year = movies_df["release_year"].max() if movies_df is not None else None

# TODO: Ex 2.3: How many director names are missing values (NaN)?
num_missing_directors = movies_df["director"].isna().sum() if movies_df is not None else None
# TODO: Ex 2.4: How many different countries are there in the data?
n_countries = movies_df["country"].nunique() if movies_df is not None else None

# TODO: Ex 2.5: How many characters long are on average the title names?
avg_title_length = movies_df["title"].str.len().mean() if movies_df is not None else None
...
top_10_countries = movies_df[movies_df["release_year"] == year]["country"].value_counts().head(10) if movies_df is not None else None
